...
public class MessageBoardDao {
	...
	public static synchronized MessageBoardDao getInstance() throws ClassNotFoundException,
	IOException{
		if(dao == null){
	...
	public String getMessageInfos(){
		...
		try {
			...
			while(rs.next()){
				...
				MessageBoardEntity entity = new MessageBoardEntity(id,userId,content,time,username,commentNum);
	...
	public String getCommentById(String messageId){
		...
		try {
			...
			while(rs.next()){
				...
				CommentEntity entity = new CommentEntity(id,t_messageId,userId,content,time);
				comments.add(entity);
	...
	public int addComment(CommentEntity entity){
		String sql = "INSERT INTO  t_comment(messagse_id,comment_user_id,comment_detail,comment_time) VALUES("
	...
	public String getMessagesByUserId(String userId){
	    ...
	    " FROM t_message m LEFT JOIN "
		+"(SELECT COUNT(message_id) AS num,id,message_id FROM t_comment GROUP BY message_id) c "
	    +"ON c.message_id = m.id  LEFT JOIN t_user u ON u.id = m.userid WHERE m.userid = '"+userId+"'";
